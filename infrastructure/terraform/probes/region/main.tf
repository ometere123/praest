...
resource "aws_iam_role_policy_attachment" "xray"{role=aws_iam_role.probe.name policy_arn="arn:aws:iam::aws:policy/AWSXRayDaemonWriteAccess"}
resource "aws_kms_key" "probe_env"{description="praest-probe-${var.region} Lambda environment variable encryption" enable_key_rotation=true}
resource "aws_lambda_function" "probe"{function_name="praest-probe-${var.region}" role=aws_iam_role.probe.arn runtime="nodejs22.x" handler="index.handler" s3_bucket=var.artifact_bucket s3_key=var.artifact_key timeout=50 memory_size=256 kms_key_arn=aws_kms_key.probe_env.arn tracing_config{mode="Active"} environment{variables={PRAEST_API_URL=var.praest_api_url,PRAEST_INTERNAL_TOKEN=var.praest_internal_token}}}
