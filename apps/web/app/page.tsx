...
const steps = ["Propose", "Accept", "Evidence", "Dispute", "GenLayer", "Finalize"];
...
export default function Home() {
  return <main className="mkt-section" style={{maxWidth: 1080, margin: "0 auto", paddingTop: 72}}>
    ...
    <div className="mkt-lifecycle" aria-label="PRAEST lifecycle">{steps.map((s, i) => <div className="mkt-lifecycle-step" key={s}><b>{String(i + 1).padStart(2, "0")} · {s}</b><span>{i === 4 ? "Bounded nondeterministic judgment" : i === 5 ? "Finalized result" : ""}</span></div>)}</div>
